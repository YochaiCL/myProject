 ...
 */
export default class UpdateSsdSata extends Component {
  state = {
  ...
  handelClick = index => {
    this.setState({
      ...
      capacity: this.state.products[index].capacity,
  ...
  async handleSubmit(e) {
    ...
    const response = await fetch(
      'http://localhost:5000/updateComponent/ssdSata',
      options
    );
    const result = await response.json();
  ...
  render() {
    return (
      <PageLayout>
        <Header h1Heading='Update SSD SATA' />
